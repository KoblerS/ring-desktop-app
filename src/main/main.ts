...
async function requestNotificationPermission(): Promise<void> {
  if (Notification.isSupported()) {
    ...
    // Show a welcome notification to trigger permission dialog on first launch
    // On macOS, this will prompt the user to allow notifications
    const settings = storageService.getSettings();
    if (settings.notificationsEnabled) {
      const testNotification = new Notification({
        title: 'Ring Desktop',
        body: 'Notifications enabled! You will receive alerts for doorbell and motion events.',
        silent: true
      });
...
ipcMain.handle('verify-2fa', async (_, email: string, password: string, code: string) => {
  try {
